...
export abstract class Inert<L extends InertItem<T>, T, O extends InertOpt<T>> implements InertLike<L, T, O> {
     ...
     * */
    protected constructor(protected leyyo: LeyyoLike, protected cluster: string, protected options: InertBuildOpt) {
        ...
        if ( !Inert._pool) {
            Inert._pool = repo.newMap<string, Inert<InertItem<unknown>, unknown, InertOpt<unknown>>>(`${where}.pool`);
        }
        ...
        Inert._pool.set(this.cluster, this._repo as unknown as Inert<InertItem<unknown>, unknown, InertOpt<unknown>>);
    }
