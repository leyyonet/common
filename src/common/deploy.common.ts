...
export class DeployCommon implements DeployCommonLike {
    ...
    constructor(private leyyo: LeyyoLike) {
        this._waitingCallbacks = this.leyyo.repoCommon.newMap<string, Array<Fnc>>(`${where}.waitingCallbacks`);
        this._alreadyDeployed = this.leyyo.repoCommon.newMap<string, Array<unknown>>(`${where}.alreadyDeployed`);
    }
