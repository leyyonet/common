...
export function setFqnObject<T = Obj | Arr>(target: T, pck: string, name: string): string {
  ...
  [
    ...
    if (!isText(value)) {
      new _leyyo.developerError(`Invalid ${field}`, testCase(PCK, 150), where).log();
      return undefined;
    }
    if (value.startsWith(".") || value.endsWith(".")) {
      new _leyyo.developerError(`Invalid ${field} with dots`, testCase(PCK, "ZZZ"), where).log();
      return undefined;
    }
  ...
  if (!target || typeof target !== "object") {
    new _leyyo.developerError(`Invalid target [${pck}, ${name}]`, testCase(PCK, 151), where).log();
    return undefined;
  }
